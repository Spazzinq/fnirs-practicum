...
for s_idx = 1:length(allsubj_results.incl_subjects)
    ...
    %% Necessary to re-sort model to match order of the test data
    % This should be handled in the classifier, but for some reason, it
    % currently isn't. For now, manually sort before starting the
    % classification.
    unique_test_labels = unique(test_labels,'stable');
    semantic_model_sortedToTest = nan(size(test_data));
    for class_num = 1:length(unique_test_labels)
        semantic_model_index = strcmp(unique_test_labels(class_num),semantic_model_labels);
        semantic_model_sortedToTest(class_num,:) = semantic_model(semantic_model_index,:);
    end
    semantic_model_labels_sortedToTest = unique_test_labels;

    %% If significance testing, permute the group labels
    ...
    for set_idx = 1:min(n_sets,allsubj_results.max_sets)
        ...
        [predicted_labels, comparisons] = allsubj_results.test_handle(...
            semantic_model_sortedToTest, ...
            semantic_model_labels_sortedToTest,...
