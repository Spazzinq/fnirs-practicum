...
nirs_folder = '/Users/conelab/Documents/BEAN_fNIRS_2024/36mo_train_peekaboo';
...
for hb_type = hb_species_list
    ...
    out_filename = sprintf('Peekaboo_36mo_chan_%s_BetweenSubjAccuracy.csv',hb_type{:});
    ...
    saveas(gcf,sprintf('figures/36mo_%s_accuracy.pdf',hb_type{:})); close gcf;
    saveas(gcf,sprintf('figures/36mo_%s_features.pdf',hb_type{:})); close gcf;
    
    fprintf('36mo, %s: overall=%0.2f, videos=%0.2f, visual=%0.2f, auditory=%0.2f\n', hb_type{:}, OverallAcc, nanmean(VideoOnly),nanmean(CarsVsFaces), nanmean(SocialVsNonsocial));
    save(['Peekaboo_36mo_' hb_type{:} '_chan_data.mat'],'MCP_struct_chan','between_subj_level')
