function trial_counts = preproc_pipeline_20241031( nirs_file, new_suffix, age )
...
% age - participant age in months
%
% Output: 
% trial_counts - a 2-dimensional array containing counts of trials found,
% rejected, or included. Columns correspond to conditions represented in
% the 's' matrix and rows indicate the following categories:
%   - valid trials before preprocessing
%   - trials automatically rejected prior to preprocessing (in input file)
%   - trials manually rejected prior to preprocessing (in input file)
%   - valid trials after preprocessing
%   - trials automatically rejected during preprocessing (by this script)
%   - trials manually rejected during preprocessing (by this script)
...
if (isstruct(nirs_file) || iscell(nirs_file)) && length(nirs_file)>1
    for file_idx = 1:length(nirs_file)
        try
            ...
            evalc('trial_counts_tmp = preproc_pipeline_20241031(nirs_file(file_idx),new_suffix,age);');
...
dRange = [3e-3,1e1]; % exclude channels with very low or high raw data (units: Volts)
SNRthreshold = 0; % SNR criterion is not used
SDrange = [0,45]; % reject Source-Detector pairs more than 45 mm apart
reset = 0;

% hmrMotionArtifactByChannel params
% These will be applied to raw data and optical density data.
if ~isfield(nirs_data,'tIncMan')
    nirs_data.tIncMan = ones(size(nirs_data.t));
end
tMotion = 1; % detect changes in 1 sec windows
tMask = 1; % mask one second of data for detected artifacts
STDEVthresh = 15; % values >15 SD above the mean are artifacts
...
% differential pathlength from Scholkmann & Wolf (2013) formula #7
% https://doi.org/10.1117/1.JBO.18.10.105004
age_years = age/12;
alpha=223.3; beta=0.05624; gamma=0.8493; delta=-5.723E-7; epsilon=0.001245; zeta=-0.9025;
nirs_data.SD.SWdpf = alpha + beta*age_years^gamma + delta*nirs_data.SD.Lambda.^3 + epsilon * nirs_data.SD.Lambda.^2 + zeta*nirs_data.SD.Lambda;
fprintf('Computed dpf=[ ');
fprintf('%0.2f ',nirs_data.SD.SWdpf);
fprintf('] for lambda=[ ');
fprintf('%03.0f ',nirs_data.SD.Lambda);
fprintf('] at age %g months\n',age);
% if age==6
%     ppf=[5.25, 4.25];
% elseif age==24
%     ppf=[5.32, 4.33];
...
nirs_data.procResult.dc = hmrOD2Conc( nirs_data.procResult.dodBP, nirs_data.SD, nirs_data.SD.SWdpf );
